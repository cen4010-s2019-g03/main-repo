...
$result = $conn->query("SELECT `username`, `first`, `last`, `user_type_description` FROM `Users` JOIN `User_Types` ON `Users`.`user_type` = `User_Types`.`user_type_id` WHERE `znum` = " . $_SESSION['znum']);
if ($result->num_rows == 1) {
	while($row = $result->fetch_assoc()) {
		$fname = $row['first'];
		$lname = $row['last'];
		$user_type = $row['user_type_description'];
...
<script>
  ...
  gtag('config', 'UA-153631369-1');
	...
	<body>
		...
		<img src="assets/profile-image-placeholder.png" class="rounded-circle border" width="150" height="150" />
		<br />
